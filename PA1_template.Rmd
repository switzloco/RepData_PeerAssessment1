...
```{r}
actSet <- read.csv("C:/Users/nswitzer/Documents/Activity Monitoring Data/activity.csv")

actSet$date <-as.Date(actSet$date,format="%Y-%m-%d")
```
...
daySum <- aggregate(steps~date,sum,na.rm=TRUE, data=actSet)
...
intervalMean <- aggregate(steps~interval,mean,na.rm=TRUE, data=actSet)

plot(intervalMean$interval,intervalMean$steps,type="l")

##Get max interval

maxInterval <- (subset(actSet,steps == max(steps,na.rm=T)))
...
missingVals <- sum(is.na(actSet$steps))

##Impute missing values

actSetFilled <- actSet

actSetFilled$steps[is.na(actSetFilled$steps)] <- median(actSetFilled$steps,na.rm=T)

#Count missing values again

daySumFilled <- aggregate(steps~date,sum,na.rm=TRUE, data=actSetFilled)
...
```{r}
library("timeDate")
actSet$weekday <- sapply(actSet$date,isWeekday,wday=1:5)
actSet$weekday <- sapply(actSet$weekday, function(x) if(x){x<-"weekday"}else{x<-"weekend"})

dayEndSum <- aggregate(steps~weekday+interval,mean,na.rm=TRUE, data=actSet)
